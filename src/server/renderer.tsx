...
export const renderer = reactRenderer(
  ({ children, routeData, initialRouteState }) => {
    const title = "Unitools: compose knowledge with ease";
    const description =
      "WYSIWYG Markdown editor for seamless content creation.";
    const routeDataJSON = escapeJSON(
      routeData ?? ({ kind: "auth" } satisfies RouteData),
    );
    const routeStateJSON = escapeJSON(
      initialRouteState ??
        ({
    ...
    return (
      <html lang="ja">
        <head>
          ...
          <script type="application/json" id="route-state">
            {routeStateJSON}
          </script>
        </head>
        <body>{children}</body>
